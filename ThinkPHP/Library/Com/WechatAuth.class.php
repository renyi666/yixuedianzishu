...
class WechatAuth {
    ...
    public function getAccessToken($type = 'client', $code = null){
        ...
        if(is_array($token)){
            if(isset($token['errcode'])){
                ...
                if($type=='client'){
                    S('access_token',$token['access_token'],1800);
                }
    ...
    public function messageCustomSend($openid, $content, $type = self::MSG_TYPE_TEXT){
        ...
        //根据类型附加额外数据
        $data[$type] = call_user_func(array(__CLASS__, $type), $content);
    ...
    public function getTicket(){
        $this->jsapiTicket          =   S('ticket');
        if(!empty($this->jsapiTicket)){
            return ;
        }
        $data['type']               =   'jsapi';
        $data['access_token']       =   $this->accessToken;
        $result                     =   $this->api('ticket/getticket', '' ,'GET',$data);
        if($result['errcode'] == 0 ){
            $this->jsapiTicket  =   $result['ticket'];
            S('ticket',$result['ticket'],7100);
        }else{
    ...
    public  function getSignArray(){
        ...
        $array['jsapi_ticket']    =   $this->jsapiTicket;
