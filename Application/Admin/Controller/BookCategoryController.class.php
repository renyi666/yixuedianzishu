...
class BookCategoryController extends AdminController {
    ...
    public function index(){
        $tree = D('BookCategory')->getTree(0,'id,title,sort,pid,status');
    ...
    public function remove(){
        ...
        $child = M('BookCategory')->where(array('pid'=>$cate_id))->field('id')->select();
        if(!empty($child)){
        ...
        //删除该分类信息
        $res = M('BookCategory')->delete($cate_id);
        if($res !== false){
            //记录行为
    ...
    public function operate($type = 'move'){
        ...
        $list = M('BookCategory')->where($map)->field('id,pid,title')->select();
        ...
        if(strcmp($type, 'move') == 0){
        	...
        	$pid = M('BookCategory')->getFieldById($from, 'pid');
    ...
    public function move(){
        $to = I('post.to');
        $from = I('post.from');
        $res = M('BookCategory')->where(array('id'=>$from))->setField('pid', $to);
        if($res !== false){
